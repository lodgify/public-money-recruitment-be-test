...
namespace VacationRental.Api.Tests
{
    ...
    public class PostRentalTests
    {
        ...
        public async Task GivenCompleteRequest_WhenPostRental_ThenAGetReturnsTheCreatedRental()
        {
            ...
            AddRentalResponse postResult;
            using (var postResponse = await _client.PostAsJsonAsync($"/api/v1/rentals", request))
            {
                Assert.True(postResponse.IsSuccessStatusCode);
                postResult = await postResponse.Content.ReadAsAsync<AddRentalResponse>();
            }
        ...
        public async Task GivenCompleteRequest_WhenPostRental_ThenWithNoBookingsAPutReturnsUpdatedRental()
        {
            ...
            AddRentalResponse rentalResponse;
            using (var rentalRequest = await _client.PostAsJsonAsync($"/api/v1/rentals", request))
            {
                ...
                rentalResponse = await rentalRequest.Content.ReadAsAsync<AddRentalResponse>();
            }
            ...
            using (var updateResponse = await _client.PutAsJsonAsync($"/api/v1/rentals/{rentalResponse.Id}", updateRequest))
            {
                ...
                var getResult = await updateResponse.Content.ReadAsAsync<UpdateRentalResponse>();
            ...
            using (var putResponse = await _client.GetAsync($"/api/v1/rentals/{rentalResponse.Id}"))
            {
                ...
                var getResult = await putResponse.Content.ReadAsAsync<GetRentalResponse>();
                Assert.Equal(updateRequest.Units, getResult.Units);
                Assert.Equal(updateRequest.PreparationTimeInDays, getResult.PreparationTimeInDays);
            }
        ...
        public async Task GivenCompleteRequest_WhenPostRental_ThenAPutReturnsErrorWhenThereisOverbooking()
        {
            ...
            var postBookings = new AddBookingRequest[] {
            ...
            foreach (AddBookingRequest bookingPost in postBookings)
            {
            ...
            var putRentalRequest = new AddRentalRequest
            {
                Units = 1,
                PreparationTimeInDays = 2
            };
        ...
        {
            var rentalRequest = new AddRentalRequest
            {
                Units = 1,
                PreparationTimeInDays = 1
            };
            ...
            {
                Assert.True(postRentalResponse.IsSuccessStatusCode);
                rentalResponse = await postRentalResponse.Content.ReadAsAsync<AddRentalResponse>();
            }
            ...
            foreach (AddBookingRequest bookingRequest in postBookings)
            {
